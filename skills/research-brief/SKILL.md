...
5. Build the in-depth brief from those findings, then derive a useful condensed answer from it. Both reading views must give the same conclusion and level of certainty. Keep any conflict, caveat, or missing evidence that materially changes the decision in the summary. More detail means explaining the scoped evidence, reasoning, and tradeoffs; it does not mean unrelated extra research, repetition, or padding.
6. Check that citations support the nearby wording and that the conclusion does not outrun the evidence. The workflow produces a brief only; it does not purchase, subscribe, post, contact people, or schedule recurring work.

## Output

Provide two reading views of the same research: **Summary** is a useful condensed answer; **In depth** is the full scoped brief with the answer, assumptions, cited findings, reasoning, conflicts/limitations, and next actions. A summary is normally two to four plain sentences, roughly 60–100 words, covering the answer, core reasons, material limits, and next action. This is a guide, not a quota or fixed wording: use less for sparse evidence or `needs_input`, and enough to preserve a consequential caveat.

In ordinary chat, default to the concise Summary view. Respect an explicit request for **Summary only**, **In depth only**, or **Both**. For Both, present distinct **Summary** and **In depth** labels without repeating the entire detailed brief in the summary. Use descriptive links for pages actually read and supplied excerpt labels for provided text in either human-readable view. Explain any incomplete verification plainly; do not invent URLs for supplied text.
...
Keep `schema_version` at `1.0` and use the existing fields without adding view fields: top-level `summary` contains the condensed answer and `data` contains the full in-depth brief. Return both together in the single dashboard response so a **Summary**/**In depth** toggle selects the corresponding content without rerunning research. A requested initial reading view does not remove the other view from dashboard JSON. For `needs_input`, put a concrete clarification question in `questions`, use a brief summary of what is missing, and keep `data` null; do not fabricate a full brief.

In JSON `summary` and `data.answer`, cite every source-backed claim or inference with individual `[source-N]` tokens that map to entries in `data.sources`. Place tokens beside the supported wording; multiple sources use adjacent tokens such as `[source-1][source-2]`, not a combined token. Keep findings' existing `source_ids` arrays as their citation mechanism. Supplied text remains `provided_text` with no invented URL or access date. Check that both reading views retain the material source conflicts and the same conditional or uncertain conclusion.
